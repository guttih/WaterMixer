...
bool connectWifiHelper(String ssid, String password, uint32_t uiDelay) {
    ...
    if (password.length() < 1) {
        ...
        outString = ssid + ".";
         ...
         outString = ssid + " with password";
        WiFi.begin(ssid.c_str(), password.c_str());
    }

    Serial.println(outString);
    pLabelHeading->setText(outString, true);
    pLabelStatus->setText("", true);
    ...
    while (status != WL_CONNECTED && iTriesLeft > 0) {
        ...
        Serial.print(status);
        pLabelStatus->setText(pLabelStatus->getText()+status, true);
        delay(uiDelay);
        iTriesLeft--;
        status = WiFi.status();
    }
...
void setup() {
    ...
    {   outString = "UNABLE to connect WiFi!";
        Serial.println(outString);
        pLabelStatus->setText(outString, true);
        ESP.restart();
    }
