...
In all three cases the response should be the following json object, (with different numbers of course) containing information about when the device was started.
...
|   GET     |  `/pins`          | Information about device pins, like pin name, pin number, 
                                  pin value and pin type.                                    | <<Get /pins response>>             |  Json 
|   POST    |  `/pins`          | Set new pin values to pins.                                | <<Post /pins request>>            |  Json 
...
|   POST    |  `/monitors`      | Add a pin monitor.                                         | <<Post /monitors request>>         |  Json
|   DELETE  |  `/monitors`      | Delete pin monitors.                                       | <<Delete /monitors request>>       |  JsonArray
|   POST    |  `/whitelist`     | Add a ip address to whitelist.                             | <<Post /whitelist request>>        |  Json 
|   DELETE  |  `/whitelist`     | Delete a ip address from whitelist.                        | <<Delete /whitelist request>>      |  Json 
|   POST    |  `/custom`        | Send commands to fill, stop, drain and change temperature 
                                  of the hot tub.                                            | <<Post /custom request>>          |  Json 
|===
...
Change pin value, in this case pin number 55 will get the value 378.
...
Current status of all pins after the change is done.
...
----
{
    ...
    "pins": [ { "pin": 50,"val": 0,  "m": 4,"name":"valveHot"},
              { "pin": 51,"val": 0,  "m": 4,"name":"valveCold"},
              { "pin": 52,"val": 0,  "m": 4,"name":"valveDrain"},
              { "pin": 53,"val": 80, "m": 4,"name":"pressure"},
              { "pin": 54,"val": 237,"m": 4,"name":"temperature"},
              { "pin": 55,"val": 378,"m": 4,"name":"desiredTemperature"}
    ]
}
----
...
----
{
    ...
    "whitelist": ["192.168.1.79", "192.168.1.198"],
    "macAddress": "F0:08:D1:C8:39:34",
    "deviceId": "61052c38bc1fdd0526aedf31",
    "hostName": "watermixer",
    "ip": "192.168.1.28",
    "port": 80,
    "date": { "year": 2023,"month": 5,   "day": 21,
              "hours": 17, "minutes": 46,"seconds": 10},
    "mixer": {
        "hotValveFlow": 0.00,
        "coldValveFlow": 0.00,
        "drainValveFlow": 0.00,
...
Monitor is an object which will watch a pin value and tell the device
to log it's value if certain conditions are met.

===== Get /monitors response

Get pin monitors.

So the response object would look something like this.
...
[
    {
        ...
        "sampleCount": 0,
        "sampleTotalCount": 10,
        "nextSampleTime": 21077329,
        "sampleInterval": 1000,
        "minLogInterval": 10000,
        "nextLogTime": 21086329
    },
    {
        "pin": 54,
        "sampleSum": 0,
        ...
        "sampleCount": 0,
        "sampleTotalCount": 10,
        "nextSampleTime": 21078329,
        "sampleInterval": 2000,
        "minLogInterval": 100000,
        "nextLogTime": 21176329
    }
]
----

===== Post /monitors request
...
[
    {
        ...
        "sampleCount": 0,
        "sampleTotalCount": 10,
        "nextSampleTime": 73486327,
        "sampleInterval": 1000,
        "minLogInterval": 10000000,
        "nextLogTime": 83485327
    }
]
----

===== Delete /monitors request

Delete a pin monitor.
...
A Whitelist contains ip addresses of devices allowed to give commands to the device.
To get the current whitelist, you can request the the status object (see: <<Get /status response>> ).

===== Post /whitelist request

Add a ip address to whitelist.

[source, json]
----
{
    "ipaddress":"185.126.62.103"
}
----

===== Post /whitelist response

Current whitelist after post
...
===== Delete /whitelist request

Delete a ip address from whitelist.
...
Current whitelist after delete
...
Commands to fill, stop, drain and change temperature of the hot tub.
 ...
 *** If hot tub is already filling, the desired temperature will change.  In addition, the hot valve flow rate will be changed to adjust the current filling temperature.
